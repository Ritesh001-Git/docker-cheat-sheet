...
<img width="3840" height="1332" alt="image" src="https://github.com/Ritesh001-Git/docker-cheat-sheet/blob/main/docker_workflow.png" />

#### Docker Runtime Flow Summary

| Step | Component  | Action |
|------|------------|--------|
| 1    | `docker` CLI | Converts user text command (e.g., `docker run ...`) into an HTTP request to the Docker Engine REST API. |
| 2    | `dockerd` (Docker Daemon) | Receives and routes API requests; manages images, containers, networks, volumes, and builds. Acts as the central control plane. |
| 3    | `containerd` | Takes the OCI bundle from `dockerd`, handles container lifecycle (start/stop/pause), image distribution, and content addressing. |
| 4    | `containerd-shim` | Stays alive after `runc` exits; "babysits" the specific container process (e.g., handles I/O, reattaches to TTY, reports exit status). Enables daemonless container management. |
| 5    | `runc` | A lightweight OCI-compliant runtime that calls Linux syscalls (`clone`, `unshare`, `setns`) to create namespaces and cgroups — bootstrapping the isolated container environment. |
| 6    | Linux Kernel | Executes the final isolated process within its namespace+cgroup constraints (e.g., `PID`, `network`, `mount`, `user`, `cgroup` namespaces + resource limits). |
